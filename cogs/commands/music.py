...
class MusicControlView(View):
    ...
    @discord.ui.button(emoji="<:voice:1408881896011862118>", style=discord.ButtonStyle.secondary)
    ...
    @discord.ui.button(emoji="<<:rewind:1408862201925730414>", style=discord.ButtonStyle.secondary)
    ...
    @discord.ui.button(emoji="<:musicstop_icons:1409190952706244631>", style=discord.ButtonStyle.success)
    async def pause_button(self, interaction: discord.Interaction, button: Button):
        if self.player.paused:
            await self.player.pause(False)

            await self.player.channel.edit(status=f"<:icons_pause:1409191035833155644> Playing: {self.player.current.title}")
            button.emoji = "<:musicstop_icons:1409190952706244631>" 
            await interaction.response.edit_message(view=self)

        elif self.player.playing:
            await self.player.pause(True)
            await self.player.channel.edit(status=f"<:icons_pause:1409191035833155644>  Paused: {self.player.current.title}")
            button.emoji = "<:rewind:1408862201925730414>"
            await interaction.response.edit_message(view=self)


    @discord.ui.button(emoji="<:skip:1409191370954117191>", style=discord.ButtonStyle.secondary)
    ...
    @discord.ui.button(emoji="<a:aLoading:1409180050049466369>", style=discord.ButtonStyle.secondary)
    ...
    @discord.ui.button(emoji="<:shuffle:1409191558506614864>", style=discord.ButtonStyle.secondary)
    ...
    @discord.ui.button(emoji="<:musicstop_icons:1409190952706244631>", style=discord.ButtonStyle.secondary)
    ...
    @discord.ui.button(emoji="<:forward:1409191789138804756>", style=discord.ButtonStyle.secondary)
...
class Music(commands.Cog):
        ...
        if len(guild.voice_channels[0].members) == 1:
            ...
            if player:
                ...
                try:
                    ...
                    ended.set_footer(text="Thanks for choosing Avalon X!")
                    support = Button(label='Support',
                                 style=discord.ButtonStyle.link,
                        url=f'https://discord.gg/5fw5zqBSh7')
    ...
    async def display_player_embed(self, player, track, ctx, autoplay=False):
        if track.artwork:
            ...
            embed.add_field(name="Source", value=f"[<:youtube:1409192094190403615>  Listen on Spotify]({track.uri})" if "spotify" in track.source else f"[<:jiosaavn:1409192198125392025> Listen on JioSaavn]({track.uri})" if "jiosaavn" in track.source else f"[<:SoundCloud:1307002774738829413> Listen on SoundCloud]({track.uri})" if "soundcloud" in track.source else f"[<:youtube:1409192094190403615> Listen on YouTube]({track.uri})")
    ...
    async def play_source(self, ctx, query):
        if not ctx.author.voice:
            await ctx.send(embed=discord.Embed(description="<:icons_warning:1408865290019668080> you need to be in a voice channel to use this command.", color=0x000000))
            return
        ...
        if isinstance(tracks, wavelink.Playlist):
            ...
            await ctx.send(embed=discord.Embed(description=f"<:icons_plus:1409181232155201647> Added playlist [{tracks.name}](https://discord.gg/5fw5zqBSh7) with **{len(tracks.tracks)} songs** to the queue.", color=0x000000))
            if not vc.playing:
            ...
            track = tracks[0]
            await vc.queue.put_wait(track)
            await ctx.send(embed=discord.Embed(description=f"<:icons_plus:1409181232155201647>  Added [{track.title}](https://discord.gg/5fw5zqBSh7) to the queue.", color=0x000000))
            if not vc.playing:
    ...
    async def handle_spotify_link(self, ctx, vc, link, type_):
        try:
            if type_ == "track":
                ...
                await ctx.send(embed=discord.Embed(description=f"<:icons_plus:1409181232155201647> Added **{c}** of **{playlist_length}** tracks from **playlist** **[{playlist_info['name']}](https://discord.gg/5fw5zqBSh7)** to the queue.", color=0x000000))
                await lmao.delete()
                ...
                await ctx.send(embed=discord.Embed(description=f"<:icons_plus:1409181232155201647> Added all tracks from album **[{album_info['name']}](https://discord.gg/5fw5zqBSh7)** to the queue.", color=0x000000))
                if not vc.playing:
                    next_track = await vc.queue.get_wait()
                    await vc.play(next_track)
                    await self.display_player_embed(vc, next_track, ctx)
    ...
    async def autoplay(self, ctx: commands.Context):
        vc = ctx.voice_client
        if not vc or not vc.playing:
            await ctx.send(embed=discord.Embed(description="<:icons_warning:1408865290019668080> No song is currently playing.", color=0x000000))
            return

        if not ctx.author.voice or ctx.author.voice.channel.id != vc.channel.id:
            await ctx.send(embed=discord.Embed(description="<:icons_warning:1408865290019668080> You need to be in the same voice channel as me to use this command.", color=0xFF0000))
            return
        ...
        if vc:
            ...
            await ctx.send(embed=discord.Embed(description=f"<:tick:1408864444796370995> Autoplay {'enabled' if vc.autoplay == wavelink.AutoPlayMode.enabled else 'disabled'} by {ctx.author.mention}.", color=0x000000))
    ...
    async def loop(self, ctx: commands.Context):
        ...
        if vc:
            ...
            await ctx.send(embed=discord.Embed(description=f"<:tick:1408864444796370995> Loop {'enabled' if vc.queue.mode == wavelink.QueueMode.loop else 'disabled'} by {ctx.author.mention}.", color=0x000000))
        else:
    ...
    async def pause(self, ctx: commands.Context):
        ...
        if vc and vc.playing and not vc.paused:
            await vc.pause(True)
            await vc.channel.edit(status=f"<:musicstop_icons:1409190952706244631> Paused: {vc.current.title}")
            await ctx.send(embed=discord.Embed(description=f"Paused by {ctx.author.mention}.", color=0x000000))
        else:
            await ctx.send(embed=discord.Embed(description="<:icons_warning:1408865290019668080>   Nothing is playing or already paused.", color=0xFF0000))
    ...
    async def resume(self, ctx: commands.Context):
        ...
        if vc and vc.paused:
            ...
            await vc.channel.edit(status=f"<:music:1408881261048631477> Playing: {vc.current.title}")
    ...
    async def skip(self, ctx: commands.Context):
        ...
        if vc and vc.playing and not vc.queue.is_empty:
            ...
            await ctx.send(embed=discord.Embed(description="<:icons_warning:1408865290019668080> No song is playing or in the queue to skip.", color=0xFF0000))
    ...
    async def volume(self, ctx: commands.Context, level: int):
        ...
        if not vc:
            await ctx.send(embed=discord.Embed(description="<:icons_warning:1408865290019668080> I'm not connected to a voice channel.", color=0xFF0000))
            return
        ...
        if vc:
            if 1 <= level <= 150:
                ...
                await ctx.send(embed=discord.Embed(description=f"<:voice:1408881896011862118> Volume set to {level}% by {ctx.author.mention}.", color=0x000000))
            else:
                await ctx.send(embed=discord.Embed(description="<:icons_warning:1408865290019668080> Volume must be between 1 and 150.", color=0xFF0000))
        else:
    ...
    async def queue(self, ctx: commands.Context):
        ...
        if not vc or not vc.queue or vc.queue.is_empty:
            await ctx.send(embed=discord.Embed(description="<:icons_warning:1408865290019668080>  The queue is currently empty.", color=0x000000))
            return
    ...
    async def clearqueue(self, ctx: commands.Context):
        ...
        if not vc or not vc.queue or vc.queue.is_empty:
            await ctx.send(embed=discord.Embed(description="<:icons_warning:1408865290019668080>  No Queue to clear.", color=0xFF0000))
            return
    ...
    async def replay(self, ctx: commands.Context):
        ...
        if not vc or not vc.playing:
            await ctx.send(embed=discord.Embed(description="<:icons_warning:1408865290019668080>  I'm not connected to any voice channel.", color=0xFF0000))
            return
    ...
    async def on_wavelink_track_start(self, payload: wavelink.TrackStartEventPayload):
        ...
        if voice_channel:
            await voice_channel.edit(status=f"<:music:1408881261048631477>  Playing: {track.title}")  # type: ignore
